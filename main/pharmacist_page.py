...
# Define file paths
icon_image = r"E:\OOP project\Hospital\media\pharmacy.ico"
logo_image = r"E:\OOP project\Hospital\media\pharmacy_logo.ico"
customer_excel_file = r"E:\OOP project\Hospital\data\customer_data.xlsx"
excel_file = r"E:\OOP project\Hospital\data\medicine_data.xlsx"
...
class PharmacyApp:
    ...
    def __init__(self):
        ...
        self.medicine_data.to_excel(excel_file, index=False)
        self.create_or_load_customer_data()
    ...
    def create_or_load_customer_data(self):
        ...
        try:
            ...
            self.customer_df = pd.DataFrame(columns=self.customer_columns)
    ...
    def add_customer(self):
        ...
        for key, value in self.customer_entries.items():
            if value.get() == "":
                messagebox.showerror("Warning", f"{key} is Missing.")
                return
        # Check if the costumer add medicine to buy or not by checking the total price
        if self.total_price == 0.0:
            messagebox.showerror("Warning", f"Please Choose Medicines You Want To Buy.")
            return
    ...
    def delete_selected_customer(self):
        ...
        if selected_items:
            for item in selected_items:
                ...
                self.customer_df = self.customer_df[self.customer_df["ID"] != customer_id]
                self.customer_df.to_excel(customer_excel_file, index=False)
    ...
    def add_medicine(self):
        ...
        for col in self.columns:
            if not len(str(self.entry_fields[col].get())):
                messagebox.showerror("Warning", f"{col} is missing")
                return
